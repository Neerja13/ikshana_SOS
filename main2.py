...
# Global variables for wave detection and alert display
wave_count = 0
last_wave_time = 0
hand_moving = None
alert_time = 0
        ...
        if result.multi_hand_landmarks:
            for hand_landmarks in result.multi_hand_landmarks:
                ...
                if detect_sos_gesture(hand_landmarks.landmark):
                    ...
                    show_alert = True
                    alert_time = time.time()  # Set the current time when alert is triggered
                    wave_count = 0  # Reset wave count after detecting SOS gesture

        # Display the SOS alert if triggered and within 3-4 seconds
        if show_alert and time.time() - alert_time < 4:  # Keep alert on screen for 4 seconds
            # Draw a large rectangle and display the alert text
            cv2.rectangle(frame, (0, 0), (frame.shape[1], 200), (0, 0, 255), -1)  # Red rectangle
            cv2.putText(frame, "   !!! ALERT !!!", (50, 100), cv2.FONT_HERSHEY_SIMPLEX,
                        2, (255, 255, 255), 4, cv2.LINE_AA)  # White text with large font
        else:
            show_alert = False  # Reset alert after 4 seconds

        # Display the frame with hand landmarks and alert
